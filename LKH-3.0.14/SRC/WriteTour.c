...
void WriteTour(char *FileName, int *Tour, GainType Cost)
{
    ...
    if (ProblemType != TSPMD && CurrentPenalty != 0 && !OptimizePenalty)
        return;
    if (FileName == 0)
        return;
    ...
    if (RouteNodes > 0) {
        ...
        fprintf(TourFile, "ROUTE_NODES : %d\n", RouteNodes);
        if (ProblemType == OP)
            fprintf(TourFile, "ROUTE_SCORE : " GainFormat "n", -RouteScore);
        fprintf(TourFile, "ROUTE_COST : %.*lf\n",
                DecimalPlaces, (double) RouteCost / Scale);
    }
...
char *FullName(char *Name, GainType Cost)
{
    ...
    if (CurrentPenalty != 0)
        snprintf(CostBuffer, 400, GainFormat "_" GainFormat,
                 CurrentPenalty, Cost);
    else
