...
/*
 * PCVRP Solution structure
 * Represents a complete solution containing tours that visit customers
 * while respecting capacity constraints and collecting prizes.
 */
struct Solution {
...
/*
 * Modified Solution structure for PCVRP
 * Represents a temporary modification to a solution during local search.
 * Tracks removed tours, new tours, and customers that were not inserted.
 */
struct ModifiedSolution {
    ...
    std::vector<int> nonInsertedNodes; // Customers that were not inserted (for prize collection)
