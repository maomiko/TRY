...
/**
 * CVRP NDSOps Python Bindings
 * 
 * This file provides Python bindings for the CVRP (Capacitated Vehicle Routing Problem)
 * C++ implementation using pybind11. It exposes the core classes and functions
 * needed for solving CVRP instances.
 */
...
/**
 * Python module definition for CVRP operations
 */
PYBIND11_MODULE(NDSOps, m) {
    m.doc() = "CVRP NDSOps - Python bindings for Capacitated Vehicle Routing Problem operations";
    ...
    py::class_<Instance>(m, "Instance")
        .def(py::init<int, int, const std::vector<int>&, const std::vector<std::vector<float>>&>(),
             "Constructor for CVRP instance",
             py::arg("numCustomers"), py::arg("vehicleCapacity"), 
             py::arg("demand"), py::arg("nodePositions"))
