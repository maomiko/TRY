/*
 * VRPTW Operations Implementation
 * 
 * Implementation of various operations for the Vehicle Routing Problem with Time Windows,
...
void sort_abs_cust(std::vector<int>& A, const Instance& instance, char order) {
    ...
    if (order == '\0') {
        static std::vector<char> options = { 'R', 'D', 'F', 'C', 'W', 'S', 'E'};
        static std::discrete_distribution<> dist({ 4, 4, 2, 1, 4, 2, 2});
    ...
    if (order == 'R') {
        ...
        // Sort by time window width (narrow to wide)
        std::sort(A.begin(), A.end(), [&](int a, int b) {
            return instance.TW_Width[a] < instance.TW_Width[b];
        });
    }
    else if (order == 'S') {
        // Sort by start time window (early to late)
        std::sort(A.begin(), A.end(), [&](int a, int b) {
            return instance.startTW[a] < instance.startTW[b];
        });
    }
    else if (order == 'E') {
        // Sort by end time window (late to early)
        std::sort(A.begin(), A.end(), [&](int a, int b) {
            return instance.endTW[a] > instance.endTW[b];
        });
    }
}
