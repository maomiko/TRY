...
/**
 * VRPTW NDSOps Python Bindings
 * 
 * This file provides Python bindings for the VRPTW (Vehicle Routing Problem with Time Windows)
 * C++ implementation using pybind11. It exposes the core classes and functions
 * needed for solving VRPTW instances with time window constraints.
 */
...
/**
 * Python module definition for VRPTW operations
 */
PYBIND11_MODULE(NDSOps, m) {
    m.doc() = "VRPTW NDSOps - Python bindings for Vehicle Routing Problem with Time Windows operations";
    ...
    py::class_<Instance>(m, "Instance")
        .def(py::init<int, int, const std::vector<int>&, const std::vector<float>&, 
                      const std::vector<float>&, const std::vector<float>&, 
                      const std::vector<std::vector<float>>&>(),
             "Constructor for VRPTW instance",
             py::arg("numCustomers"), py::arg("vehicleCapacity"), 
             py::arg("demand"), py::arg("startTW"), py::arg("endTW"), 
             py::arg("serviceTime"), py::arg("nodePositions"))
    ...
    py::class_<Solution>(m, "Solution")
        .def(py::init<const Instance&, const std::vector<std::vector<int>>&>(),
             "Constructor for VRPTW solution",
